...
export async function middleware(request: NextRequest) {
  ...
  let localeResponse = NextResponse.redirect(redirectUrl, 307);
  ...
  if (!urlHasCountryCode && countryCode) {
    ...
    localeResponse = NextResponse.redirect(`${redirectUrl}`, 307);

    localeResponse.cookies.set('_medusa_cache_id', cacheId, {
      maxAge: 60 * 60 * 24,
    });
