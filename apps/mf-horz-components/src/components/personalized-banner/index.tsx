...
function PoweredBySanity({ className }: { className: string }) {
  return (
...
function BannerButton({
  ...
  if (isExternalLink) {
    return (
      <a href={action.href} target={target} rel={rel}>
        {button}
      </a>
    );
  }
              ...
              {(title || description) && (
                <div className="mt-4 flex flex-col gap-3">
                  {title && (
                    <Heading
                      level="h1"
                      className="text-ui-fg-base text-4xl font-normal leading-tight [text-shadow:0_1px_14px_rgba(255,255,255,0.4)] md:text-5xl"
                    >
                      {title}
                    </Heading>
                  )}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {visibleSecondaryBanners.map((banner, index) => (
              ...
              >
                ...
                <div
                  className="via-black/24 to-black/12 absolute inset-0 bg-gradient-to-t from-black/60"
                  aria-hidden="true"
                />
                ...
                <div className="relative flex flex-col items-start gap-2.5 sm:gap-3">
                  {banner.title && (
                    <Heading
                      level="h2"
                      className="text-ui-fg-on-color text-xl font-normal leading-tight [text-shadow:0_2px_18px_rgba(0,0,0,0.45)] sm:text-2xl md:text-3xl"
                    >
                      {banner.title}
                    </Heading>
                  )}
